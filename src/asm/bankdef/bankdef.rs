use diagn::Span;
use syntax::{Parser, Token, TokenKind};
    ...
    pub fn parse(&mut self) -> Result<(), ()> {
        ...
        if self.addr.is_none() {
            return Err(self
                .parser
                .report
                .error_span("missing #addr attribute", &self.span));
        }
        ...
        if self.size.is_none() {
            return Err(self
                .parser
                .report
                .error_span("missing #size attribute", &self.span));
        }
    ...
    pub fn parse_attribute(&mut self) -> Result<(), ()> {
        ...
        match attrb_name.as_ref() {
            ...
            _ => Err(self
                .parser
                .report
                .error_span("unknown attribute", &tk_attrb_name.span)),
        }
    }
    ...
    pub fn parse_attribute_addr(&mut self, tk_attrb_name: &Token) -> Result<(), ()> {
        if self.addr.is_some() {
            return Err(self
                .parser
                .report
                .error_span("duplicate #addr attribute", &tk_attrb_name.span));
        }
    ...
    pub fn parse_attribute_size(&mut self, tk_attrb_name: &Token) -> Result<(), ()> {
        if self.size.is_some() {
            return Err(self
                .parser
                .report
                .error_span("duplicate #size attribute", &tk_attrb_name.span));
        }
    ...
    pub fn parse_attribute_outp(&mut self, tk_attrb_name: &Token) -> Result<(), ()> {
        if self.outp.is_some() {
            return Err(self
                .parser
                .report
                .error_span("duplicate #outp attribute", &tk_attrb_name.span));
        }
    ...
    pub fn parse_attribute_fill(&mut self, tk_attrb_name: &Token) -> Result<(), ()> {
        if self.fill.is_some() {
            return Err(self
                .parser
                .report
                .error_span("duplicate #fill attribute", &tk_attrb_name.span));
        }
