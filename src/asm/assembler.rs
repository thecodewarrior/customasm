...
use asm::BankDef;
use asm::BinaryBlock;
use asm::{AssemblerParser, BinaryOutput, FunctionManager, LabelContext, LabelManager};
...
impl AssemblerState {
        ...
        for j in 1..self.bankdefs.len() {
            ...
            for i in 1..j {
                ...
                if outp1 + bank1.size > outp2 && outp1 < outp2 + bank2.size {
                    report.error_span(
                        format!(
                            "banks `{}` and `{}` overlap in output location",
                            bank1.name, bank2.name
                        ),
                        &bank1.decl_span.as_ref().unwrap(),
                    );
                }
            }
        }
    ...
    pub fn get_cur_address(&self, report: RcReport, span: &Span) -> Result<usize, ()> {
        ...
        if excess_bits != 0 {
            ...
            return Err(report.error_span(
                format!(
                    "address is not aligned to a word boundary ({} {} short)",
                    bits_short, plural
                ),
                span,
            ));
    ...
    pub fn output_parsed_expr(
        ...
        if value_width > expr.width {
            let descr = format!(
                "value (width = {}) is larger than the specified width; use a bit slice",
                value_width
            );
    ...
    fn expr_eval_fn(
        ...
        match fn_name {
            "assert" => {
                if args.len() != 1 {
                    return Err({
                        report.error_span("wrong number of arguments", span);
                        true
                    });
                }

                return match args[0] {
                    ExpressionValue::Bool(value) => match value {
                        true => Ok(ExpressionValue::Void),
                        false => Err({
                            report.error_span("assertion failed", span);
                            true
                        }),
                    },

                    _ => Err({
                        report.error_span("wrong argument type", span);
                        true
                    }),
                };
            }

            _ => match self.functions.get_func(fn_name, args.len()) {
                Some(func) => {
                    let mut args_eval_ctx = ExpressionEvalContext::new();
                    for i in 0..args.len() {
                ...
                None => {
                    return Err({
                        report.error_span("unknown function", span);
                        true
                    });
                }
            },
...
impl ExpressionContext {
    pub fn get_address_at(
        ...
        if block.len() % bits != 0 {
            return Err({
                report.error_span("address is not aligned to a byte", span);
                true
            });
        }
        ...
        match block_offset.checked_add(state.bankdefs[bankdef].addr) {
            Some(addr) => Ok(addr),
            None => Err({
                report.error_span("address overflowed valid range", span);
                true
            }),
        }
    }
