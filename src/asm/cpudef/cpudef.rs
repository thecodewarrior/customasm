...
impl CpuDef {
        ...
        S: Into<String>,
    {
        ...
        std::mem::swap(
            &mut parser.custom_token_defs,
            &mut sub_parser.custom_token_defs,
        );
...
impl<'t> CpuDefParser<'t> {
    ...
    fn parse_directive_align(&mut self, tk_name: &Token) -> Result<(), ()> {
        self.parser
            .report
            .warning_span("deprecated directive; use `#bits`", &tk_name.span);
    ...
    fn parse_directive_bits(&mut self, tk_name: &Token) -> Result<(), ()> {
        ...
        if self.bits.is_some() {
            return Err(self
                .parser
                .report
                .error_span("duplicate `bits` directive", &tk_name.span));
        }
        ...
        if bits == 0 {
            return Err(self
                .parser
                .report
                .error_span("invalid byte size", &tk_bits.span));
        }
    ...
    fn parse_directive_labelalign(&mut self, tk_name: &Token) -> Result<(), ()> {
        ...
        if self.label_align.is_some() {
            return Err(self
                .parser
                .report
                .error_span("duplicate `labelalign` directive", &tk_name.span));
        }
        ...
        if value == 0 {
            return Err(self
                .parser
                .report
                .error_span("invalid alignment", &tk_value.span));
        }
    ...
    fn parse_directive_tokendef(&mut self, _tk_name: &Token) -> Result<(), ()> {
        ...
        if self
            .custom_token_defs
            .iter()
            .find(|def| def.name == defname)
            .is_some()
        {
            return Err(self
                .parser
                .report
                .error_span("duplicate tokendef name", &tk_defname.span));
        }
    ...
    fn parse_tokendef_token(&mut self, tokendef: &mut CustomTokenDef) -> Result<(), ()> {
        ...
        while !self.parser.is_over()
            && !self.parser.next_is(0, TokenKind::Colon)
            && !self.parser.next_is(0, TokenKind::BraceClose)
        {
            ...
            if !tk.kind.is_allowed_pattern_token() {
                return Err(self
                    .parser
                    .report
                    .error_span("invalid pattern token", &tk.span));
            }
            if is_first_token {
        ...
        if tokendef.tokens.contains_key(&tokens) {
            return Err(self
                .parser
                .report
                .error_span("duplicate tokendef entry", &full_span));
        }
        ...
        while !self.parser.next_is(0, TokenKind::FatArrow) {
            ...
            if tk.kind == TokenKind::BraceOpen {
                ...
                if prev_was_expr_parameter {
                    return Err(self.parser.report.error_span(
                        "expected a separating token between parameters; try using a `,`",
                        &tk.span.before(),
                    ));
                }
            ...
            // Read an exact pattern part.
                ...
                if prev_was_expr_parameter && !tk.kind.is_allowed_after_pattern_parameter() {
                    return Err(self.parser.report.error_span(
                        "potentially ambiguous token after parameter; try using a separating `,`",
                        &tk.span,
                    ));
                }
            ...
            if is_beginning_of_pattern && !is_allowed_at_beginning {
                return Err(self.parser.report.error_span(
                    "expected identifier as first pattern token",
                    &tk.span.before(),
                ));
            }
        }
    ...
    fn parse_rule_parameter(&mut self, rule: &mut Rule) -> Result<bool, ()> {
        ...
        if is_reserved_var(&name) {
            return Err(self
                .parser
                .report
                .error_span("reserved variable name", &tk_name.span));
        }
        ...
        if rule.param_exists(&name) {
            return Err(self
                .parser
                .report
                .error_span("duplicate parameter name", &tk_name.span));
        }

        let (is_expr_parameter, typ) = if self.parser.maybe_expect(TokenKind::Colon).is_some() {
            ...
            if tokendef_index.is_none() {
                return Err(self
                    .parser
                    .report
                    .error_span("unknown parameter type", &tk_type.span));
            }
