    ...
    T: Into<String>,
{
    ...
    let adjusted_result = if let Fail((filename, line, msg)) = expected {
...
fn test_fileserver<S>(
    fileserver: &FileServer,
    asm_filename: S,
    expected: ExpectedResult<(usize, &'static str)>,
) where
    S: Into<String>,
{
    ...
    let assemble =
        |report: RcReport, fileserver: &FileServer, filename: S| -> Result<(usize, String), ()> {
    ...
    expect_result(
        report.clone(),
        fileserver,
        result.as_ref().map(|r| (r.0, r.1.as_ref())).ok(),
        expected,
    );
}
...
fn test_simple() {
    ...
    test(
        "#bits 128 \n halt -> ((1 << 256) / 0xfedc)[255:0]",
        "halt",
        Pass((
            4,
            "000101254e8d998319892068f7ba90cd2a03ec79bad91fa81bbfa69a07b0c5a1",
        )),
    );
...
fn test_parameters() {
    ...
    test(
        "load {a}, {b} -> 0x12 @ a[7:0] @ b[7:0]",
        "load 0x34, 0x56",
        Pass((4, "123456")),
    );
    test(
        "load {a}, {b} -> 0x12 @ b[7:0] @ a[7:0]",
        "load 0x34, 0x56",
        Pass((4, "125634")),
    );

    test(
        "load {a}      -> 0x12 @ (a +  0x22)[7:0]",
        "load 0x34",
        Pass((4, "1256")),
    );
    test(
        "load {a}      -> 0x12 @ (a + 0xf22)[7:0]",
        "load 0x34",
        Pass((4, "1256")),
    );
    test(
        "load {a}, {b} -> 0x12 @ (a + b)[7:0]",
        "load 0x34, 0x56",
        Pass((4, "128a")),
    );
...
fn test_tokendef() {
    test(
        "#tokendef reg { r1 = 1    } \n mov {a: reg} -> 0xff @ a[7:0]",
        "mov r1",
        Pass((4, "ff01")),
    );
    test(
        "#tokendef reg { r1 = 0xbc } \n mov {a: reg} -> 0xff @ a[7:0]",
        "mov r1",
        Pass((4, "ffbc")),
    );

    test(
        "#tokendef reg { r1 = 1, r2 = 2 } \n mov {a: reg} -> 0xff @ a[7:0]",
        "mov r1 \n mov r2",
        Pass((4, "ff01ff02")),
    );
    ...
    test(
        "#tokendef reg { r1 = 1 } \n mov [{a: reg} + {offset}] -> 0xff @ a[7:0] @ offset[7:0]",
        "mov [r1 + 8]",
        Pass((4, "ff0108")),
    );
    ...
    test(
        "#tokendef reg { r1 = 1, r1 = 2 } \n mov {a: reg} -> 0xff @ a[7:0]",
        "mov r1",
        Fail(("cpu", 1, "duplicate tokendef entry")),
    );
    test(
        "#tokendef 123 { r1 = 1, r2 = 2 } \n mov {a: reg} -> 0xff @ a[7:0]",
        "mov r1",
        Fail(("cpu", 1, "identifier")),
    );
    test(
        "#tokendef reg { r1 = 1 } \n #tokendef reg { r2 = 1 } \n mov {a: reg} -> 0xff @ a[7:0]",
        "mov r1",
        Fail(("cpu", 2, "duplicate tokendef name")),
    );
    ...
    test(
        "#tokendef alu { add = 0x10, sub = 0x20 } \n {op: alu} {x} -> op[7:0] @ x[7:0]",
        "add 0xef \n sub 0xfe",
        Pass((4, "10ef20fe")),
    );
}
...
fn test_assertions() {
    test(
        "load {a} -> { assert(a % 2 == 0), 0x12 @ a[7:0] }",
        "load 0x34",
        Pass((4, "1234")),
    );
    test(
        "load {a} -> { assert(a % 2 == 0), 0x12 @ a[7:0] }",
        "load 0x23",
        Fail(("cpu", 1, "assertion")),
    );
    test(
        "load {a} -> { assert(pc >= 0x02), 0x12 @ a[7:0] }",
        "load 0x34",
        Fail(("cpu", 1, "assertion")),
    );
    test(
    ...
    test(
        "halt -> { assert(pc % 2 == 0), 0x12 @ pc[7:0] }",
        "halt \n halt \n halt",
        Pass((4, "120012021204")),
    );
    test(
...
fn test_addr_directive() {
    ...
    test(
        "halt -> 0x12",
        "#addr 0x10 \n halt",
        Pass((4, "0000000000000000000000000000000012")),
    );
    ...
    test(
        "halt -> 0x12",
        "#addr 0x10 \n halt \n #addr 0x12",
        Pass((4, "000000000000000000000000000000001200")),
    );
    ...
    test(
        "halt -> 0x12 @ pc[7:0]",
        "#addr 0x10 \n halt",
        Pass((4, "000000000000000000000000000000001210")),
    );
    ...
    test(
        "halt -> 0x12 @ pc[7:0]",
        "halt \n halt \n #addr 0x10 \n halt \n halt",
        Pass((4, "1200120200000000000000000000000012101212")),
    );
...
fn test_data_directive() {
    ...
    test(
        "#bits 5",
        "#d5 1, 2, 3, -1",
        Pass((1, "00001000100001111111")),
    );
    test(
        "#bits 7",
        "#d7 1, 2, 3, -1",
        Pass((1, "0000001000001000000111111111")),
    );
    test(
        "#bits 9",
        "#d9 1, 2, 3, -1",
        Pass((1, "000000001000000010000000011111111111")),
    );
    ...
    test(
        "",
        "              #d8 x + pc \n #addr 0x10 \n x = 0x11",
        Pass((4, "11000000000000000000000000000000")),
    );
    test(
        "",
        "#addr 0x10 \n #d8 x + pc \n               x = 0x11",
        Pass((4, "0000000000000000000000000000000021")),
    );
...
fn test_str_directive() {
    ...
    test(
        "",
        "#str \"\\u{7f}\\u{80}\\u{ff}\\u{10ffff}\"",
        Pass((4, "7fc280c3bff48fbfbf")),
    );
    ...
    test(
        ...
        Pass((
            1,
            "01100001011000100110001100110010001100101011001100110011101101000",
        )),
    );
    test(
        "#bits 32",
...
fn test_labels() {
    ...
    test(
        INSTRSET,
        "start: halt \n .br: jump .br \n #addr 0x08 \n mid: halt \n .br: jump .br",
        Pass((4, "1277010000000000127709")),
    );

    test(
        INSTRSET,
        " label: halt \n  label: halt",
        Fail(("asm", 2, "duplicate")),
    );
    test(
        INSTRSET,
        ".label: halt \n .label: halt",
...
fn test_include_directive() {
    ...
    test_fileserver(
        ...
        Pass((
            4,
            "12003400120412063400120a34003406340a121234003406340a3412121c34003406340a3412",
        )),
    );
...
fn test_incstr_directives() {
    ...
    test_fileserver(
        &fileserver,
        "main1_3",
        Fail(("main1_3", 2, "invalid character")),
    );
    test_fileserver(&fileserver, "main1_4", Pass((4, "0123456789abcdef")));
    test_fileserver(
        &fileserver,
        "main1_5",
        Fail(("main1_5", 2, "invalid character")),
    );
    test_fileserver(
        &fileserver,
        "main1_6",
        Fail(("main1_6", 2, "invalid character")),
    );
    ...
    test_fileserver(
        &fileserver,
        "main2_3",
        Fail(("main2_3", 2, "invalid character")),
    );
    test_fileserver(&fileserver, "main2_4", Fail(("main2_4", 3, "align")));
    test_fileserver(
        &fileserver,
        "main2_5",
        Fail(("main2_5", 2, "invalid character")),
    );
    test_fileserver(
        &fileserver,
        "main2_6",
        Fail(("main2_6", 2, "invalid character")),
    );
    ...
    test_fileserver(
        &fileserver,
        "main3_3",
        Fail(("main3_3", 2, "invalid character")),
    );
    test_fileserver(&fileserver, "main3_4", Pass((4, "0123456789abcdef")));
    test_fileserver(
        &fileserver,
        "main3_5",
        Fail(("main3_5", 2, "invalid character")),
    );
    test_fileserver(
        &fileserver,
        "main3_6",
        Fail(("main3_6", 2, "invalid character")),
    );
...
fn test_banks() {
    ...
    test(
        "",
        "#bankdef \"hello\" { #addr 0 }",
        Fail(("asm", 1, "missing")),
    );
    test("", "#bankdef \"hello\" { #addr 0, #size 0 }", Pass((4, "")));
    test(
        "",
        "#bankdef \"hello\" { #addr 0, #size 0, #outp 0 }",
        Pass((4, "")),
    );
    test(
        "",
        "#bankdef \"hello\" { #addr 0, #size 0,          #fill }",
        Pass((4, "")),
    );
    test(
        "",
        "#bankdef \"hello\" { #addr 0, #size 0, #outp 0, #fill }",
        Pass((4, "")),
    );
    ...
    test(
        "",
        "#d8 0xff \n #bankdef \"hello\" { #addr 0, #size 0, #outp 0 }",
        Fail(("asm", 2, "default bank")),
    );

    test(
        "",
        "#bankdef \"hello\" { #addr 0, #size 10, #outp 0        }",
        Pass((4, "")),
    );
    test(
        "",
        "#bankdef \"hello\" { #addr 0, #size 10, #outp 0, #fill }",
        Pass((4, "00000000000000000000")),
    );

    test(
        "",
        "#bankdef \"hello\" { #addr 0, #size 10 } \n #res 1",
        Pass((4, "")),
    );
    test(
        "",
        "#bankdef \"hello\" { #addr 0, #size 10 } \n #d8 0",
        Fail(("asm", 2, "non-writable")),
    );
    ...
    test(
        "",
        "#bankdef \"hello\" { #addr 10, #size 10 } \n #res 1 \n #addr 0 \n #res 1",
        Fail(("asm", 3, "out of bank range")),
    );

    test(
        "",
        "#bankdef \"hello\" { #addr 0, #size 4, #outp 0 }
    ...
    test(
        "",
        "#bankdef \"hello\" { #addr 0, #size 4, #outp 3 }
	          #bankdef \"world\" { #addr 0, #size 4, #outp 0 }",
        Fail(("asm", 1, "overlap")),
    );
    ...
    test(
        "",
        "#bankdef \"hello\" { #addr 0, #size 4, #outp 4 }
	          #bankdef \"world\" { #addr 0, #size 4, #outp 0 }",
        Pass((4, "")),
    );
    ...
    test(
        "",
        "#bankdef \"hello\" { #addr 7, #size 4, #outp 0 }
	          #d8 pc, 1, 2, 3",
        Pass((4, "07010203")),
    );
    ...
    test(
        "",
        "#bankdef \"hello\" { #addr 7, #size 4, #outp 4 }
	          #d8 pc, 1, 2, 3",
    ...
    test(
        "",
        "#bankdef \"hello\" { #addr 0x00, #size 0x08, #outp 0x00 }
