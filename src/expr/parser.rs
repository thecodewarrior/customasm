use super::{BinaryOp, Expression, ExpressionValue, UnaryOp};
use syntax::{excerpt_as_bigint, excerpt_as_usize, Parser, TokenKind};
...
impl<'a> ExpressionParser<'a> {
    ...
    fn parse_unary_ops<F>(
        &mut self,
        ops: &[(TokenKind, UnaryOp)],
        parse_inner: F,
    ) -> Result<Expression, ()>
    where
        F: Fn(&mut ExpressionParser<'a>) -> Result<Expression, ()>,
    {
        for op in ops {
    ...
    fn parse_assignment(&mut self) -> Result<Expression, ()> {
        self.parse_right_associative_binary_ops(&[(TokenKind::Equal, BinaryOp::Assign)], |s| {
            s.parse_concat()
        })
    }

    fn parse_concat(&mut self) -> Result<Expression, ()> {
        self.parse_binary_ops(&[(TokenKind::At, BinaryOp::Concat)], |s| s.parse_lazy_or())
    }
    ...
    fn parse_lazy_and(&mut self) -> Result<Expression, ()> {
        self.parse_binary_ops(&[(TokenKind::AmpersandAmpersand, BinaryOp::LazyAnd)], |s| {
            s.parse_relational()
        })
    }
    ...
    fn parse_binary_or(&mut self) -> Result<Expression, ()> {
        self.parse_binary_ops(&[(TokenKind::VerticalBar, BinaryOp::Or)], |s| {
            s.parse_binary_xor()
        })
    }

    fn parse_binary_xor(&mut self) -> Result<Expression, ()> {
        self.parse_binary_ops(&[(TokenKind::Circumflex, BinaryOp::Xor)], |s| {
            s.parse_binary_and()
        })
    }

    fn parse_binary_and(&mut self) -> Result<Expression, ()> {
        self.parse_binary_ops(&[(TokenKind::Ampersand, BinaryOp::And)], |s| {
            s.parse_shifts()
        })
    }
    ...
    fn parse_bitslice(&mut self) -> Result<Expression, ()> {
        ...
        let leftmost = excerpt_as_usize(
            self.parser.report.clone(),
            tk_leftmost.excerpt.as_ref().unwrap(),
            &tk_leftmost.span,
        )?;
        let rightmost = excerpt_as_usize(
            self.parser.report.clone(),
    ...
    fn parse_number(&mut self) -> Result<Expression, ()> {
        ...
        let (bigint, width, radix, digit_num) =
            excerpt_as_bigint(self.parser.report.clone(), &number, &tk_number.span)?;
