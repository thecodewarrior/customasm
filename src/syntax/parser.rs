use diagn::RcReport;
use syntax::{excerpt_as_usize, Token, TokenKind};
...
impl Parser {
    ...
    fn skip_ignorable(&mut self) {
        while self.index < self.tokens.len() - 1 && self.tokens[self.index].kind.ignorable() {
        ...
        S: Into<String>,
    {
        match self.maybe_expect(kind) {
            ...
            None => Err(self
                .report
                .error_span(descr, &self.tokens[self.index_prev].span.after())),
        }
    }
